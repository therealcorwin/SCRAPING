...
logger.remove()
logger.add(f"books.log", level="WARNING", rotation="500kb")
...
def get_all_books_urls_on_page(url: str, tree: HTMLParser) -> List[str]:
    ...
    try:
        ...
        urls = [
            urljoin(url, link.attributes.get("href"))
        ...
        logger.error(
            f"Erreur lors de l'extraction des URLs des livres sur la page {url} : {e}"
        )
        return []
